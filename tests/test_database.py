...
class TestDatabaseManager:
    ...
    def test_get_conversation_history(self, db_manager):
        ...
        db_manager.supabase.rpc.assert_called_with('get_conversation_history', {'schema_name': TEST_SESSION_ID})
        assert len(result) == 2
    ...
    def test_add_interaction_rpc_failure_fallback(self, db_manager):
        ...
        assert result is True
        db_manager.supabase.table.assert_called_with(f"{TEST_SESSION_ID}.interactions")
        ...
        mock_response.data = [
            {
                ...
                'metadata': {'topic': 'Anxiety', 'effectiveness': {'term_overlap': 0.9}}
            }
        ]
        db_manager.supabase.rpc().execute.return_value = mock_response
        
        # Call method
        result = db_manager.get_high_quality_interactions(topic_filter='Anxiety')
        
        # Verify result and SQL command
        assert len(result) == 1
        assert result[0]['question'] == 'How do I manage anxiety?'
        
        # Check that SQL command was correct
        sql_call_args = db_manager.supabase.rpc.call_args[1]
        assert 'sql' == db_manager.supabase.rpc.call_args[0][0]
        assert "metadata->>'topic' = 'Anxiety'" in sql_call_args['command']
        assert "CAST(metadata->'effectiveness'->>'term_overlap' AS FLOAT) >= 0.7" in sql_call_args['command']
    ...
    def test_connect_psychological_concepts(self, db_manager):
        ...
        # Mock the rpc execution for table creation
        mock_table_response = Mock()
        mock_table_response.error = None
        
        # Mock the table insertion
        mock_insert_response = Mock()
        mock_insert_response.error = None
        
        # Configure mocks
        db_manager.supabase.rpc().execute.return_value = mock_table_response
        db_manager.supabase.table().insert().execute.return_value = mock_insert_response
        ...
        assert result is True
        db_manager.supabase.table.assert_called_with(f"{TEST_SCHEMA}.concept_connections")
        
        # Check the connection data that was inserted
        table_call = db_manager.supabase.table()
        insert_call = table_call.insert
    ...
    def test_connect_psychological_concepts_error(self, db_manager):
        ...
        db_manager.supabase.table().insert().execute.side_effect = Exception("Connection error")
    ...
    def test_analyze_emotional_trajectory(self, db_manager):
        ...
        emotional_history = [
            ...
            {
                'questionText': 'How did the breathing exercises work for you?',
                'answerText': 'They helped a bit. I'm still anxious but less than before.',
                'metadata': json.dumps({
                    'emotional_state': 'anxious',
            ...
            {
                'questionText': 'How are you feeling now about your progress?',
                'answerText': 'I feel more hopeful and my anxiety is more manageable.',
                'metadata': json.dumps({
        ...
        with patch.object(db_manager, 'get_conversation_history', return_value=emotional_history):
            # Call method
            result = db_manager.analyze_emotional_trajectory(TEST_SESSION_ID)
            
            # Verify trajectory shows emotional progression
            assert len(result) == 3
            assert result[0]['emotional_state'] == 'anxious'
            assert result[0]['intensity'] > result[1]['intensity']
            assert result[2]['emotional_state'] == 'hopeful'
            assert result[2]['intensity'] < result[0]['intensity']
            
            # Verify chronological ordering
            timestamps = [entry.get('timestamp', '') for entry in result]
            assert timestamps == sorted(timestamps)
    
    def test_analyze_emotional_trajectory_empty(self, db_manager):
    ...
    def test_analyze_emotional_trajectory_invalid_metadata(self, db_manager):
        ...
        with patch.object(db_manager, 'add_interaction', return_value=True):
            ...
            expected_data = {
                ...
                'metadata': {
                    ...
                    'session_number': 3,
                    'session_start': True,
                    'session_timestamp': patch.ANY  # We don't know exact timestamp
                }
            }
    ...
    def test_mark_therapeutic_insight(self, db_manager):
        ...
        # Verify result and RPC call
        assert result is True
        db_manager.supabase.rpc.assert_called_with('execute_sql', {'command': patch.ANY})
    ...
    def test_mark_therapeutic_insight_null_metadata(self, db_manager):
        ...
        # Verify SQL contains null handling
        command = db_manager.supabase.rpc.call_args[1]['command']
        assert "WHEN metadata IS NULL THEN '{}'" in command
        assert f'"{TEST_SCHEMA}".interactions' in command
