...
class TestEmbeddingGeneration(DatabaseTestBase):
    ...
    def test_tokenizer_output_handling(self):
        ...
        mock_encoding = {"input_ids": torch.tensor([[1, 2, 3]]), "attention_mask": torch.tensor([[1, 1, 1]])}
    ...
    @patch.object(ModelManager, "get_generator", autospec=True)
    def test_model_manager_get_embedding_integration(self, mock_get_generator):
        """Test calling get_embedding via ModelManager using a mocked generator."""
        mock_generator_instance = MagicMock(spec=TextGenerator)  # Use spec for better mocking

        # Mock the get_embedding method to return a fixed tensor
        mock_embedding = torch.tensor([[0.1] * 768], dtype=torch.float32)
        mock_generator_instance.get_embedding.return_value = mock_embedding

        # Configure the mocked ModelManager.get_generator to return the instance
        mock_get_generator.return_value = mock_generator_instance

        # Instantiate ModelManager (its get_generator is now mocked)
        with patch.object(ModelManager, "__init__", return_value=None):  # Avoid real init
            model_manager = ModelManager(model_name="mock-model")

        # Call the mocked get_generator method
        retrieved_generator = model_manager.get_generator()

        # Verify that get_generator returned our mock instance
        self.assertIs(retrieved_generator, mock_generator_instance)

        # Test get_embedding directly on the retrieved generator
        test_text = "Test this embedding call via ModelManager"
        embedding = retrieved_generator.get_embedding(test_text)

        # Verify results
        self.assertIsNotNone(embedding)
        self.assertTrue(torch.equal(embedding, mock_embedding), "Embedding should match mock embedding")
        # Check that the get_embedding method was called
        mock_generator_instance.get_embedding.assert_called_once_with(test_text)
        # Check that ModelManager.get_generator was called
        mock_get_generator.assert_called_once_with(model_manager)
    ...
    def test_dict_context_storage_edge_cases(self):
        ...
        nested_context = {
            ...
            "metadata": {
                ...
                "deep": {"nested": {"value": [1, 2, None, "text"]}},
                "empty_dict": {},
                "none_value": None,
            },
            "embedding": [0.1, 0.2, 0.3],
    ...
    def test_embedding_end_to_end(self, mock_get_provider):
        ...
        context = {
            ...
            "psychological_context": {"topic": "anxiety", "emotion": "fear"},
        }
