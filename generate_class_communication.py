...
import re
import sys
from typing import Any, Dict, List, Set, Tuple, Type, TypedDict

import pydot


# Define TypedDicts for better structure understanding
class ClassAttributeDetail(TypedDict):
    name: str
    type: str
...
class ClassInfo(TypedDict):
    module: str
    class_obj: Type[Any]
    attributes: List[ClassAttributeDetail]
    methods: List[str]
...
class CommunicationInfo(TypedDict):
    from_class: str
    to_class: str
    type: str  # "composition" or "method_call"
    methods: List[str]


class ExtractedData(TypedDict):
    classes: Dict[str, ClassInfo]
    communications: List[CommunicationInfo]
...
def extract_classes_and_communications(modules: List[str]) -> ExtractedData:
    ...
    for class_name, class_info in classes.items():
        try:
            # Get class source
            source = inspect.getsource(class_info["class_obj"])
            ...
            for attr_name, attr_class in instance_matches:
                if attr_class in classes:
                    class_info["attributes"].append({"name": attr_name, "type": attr_class})

                    # Add communication link (composition)
                    communications.append(
                        {"from_class": class_name, "to_class": attr_class, "type": "composition", "methods": []}
                    )
                ...
                for attr in class_info["attributes"]:
                    if attr["name"] == instance_name:
                        ...
                        for comm in communications:
                            if (
                                comm["from_class"] == class_name
                                and comm["to_class"] == attr["type"]
                                and comm["type"] == "method_call"
                            ):
    ...
    return {"classes": classes, "communications": communications}


def generate_communication_diagram(data: ExtractedData) -> pydot.Dot:
    ...
    graph.set_node_defaults(
        shape="box", style="filled", fontname="Arial", fontsize="12", height="0.6", margin="0.3,0.1"
    )
    ...
    for module, classes in module_classes.items():
        ...
        for class_name in classes:
            ...
            node = pydot.Node(class_name, label=node_label, shape="record", fillcolor="white")
...
def generate_focused_diagram(data: ExtractedData, focal_class: str) -> pydot.Dot:
    ...
    for comm in data["communications"]:
        ...
        if (
            from_class in class_nodes
            and to_class in class_nodes
            and (from_class == focal_class or to_class == focal_class)
        ):
...
def main() -> None:
    ...
    key_classes = ["DynamicRAGRetriever", "DatabaseManager", "ModelManager", "AssociativeMemory", "TextGenerator"]
